...
export async function getPayment(userId: number, ticketId: number) {
  const ticket = await ticketsRepository.ticketById(ticketId);
  if (!ticket) {
    throw notFoundError();
  }
  const enrollment = await enrollmentRepository.enrollmentByUserId(userId);
...
export async function processPayment(userId: number, ticketId: number, cardData: CardData) {
  ...
  const tickeType = await ticketsRepository.typeById(ticket.ticketTypeId);
